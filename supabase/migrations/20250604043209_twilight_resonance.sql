...
DECLARE
  result json;
  most_popular_course record;
BEGIN
  -- Find most popular course
  SELECT 
    c.id,
    c.title,
    COUNT(DISTINCT ump.user_id) as students_count
  INTO most_popular_course
  FROM courses c
  ...
  IF most_popular_course.id IS NULL THEN
    most_popular_course := ROW('1', 'Introduction à l''IA', 248);
  END IF;
  ...
  SELECT json_build_object(
    ...
    'most_popular_course', json_build_object(
      'id', most_popular_course.id,
      'title', most_popular_course.title,
      'students_count', most_popular_course.students_count
    )
  ) INTO result;
